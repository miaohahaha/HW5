...
public class Solution2C1Activity extends AppCompatActivity {
    ...
    public void requestData(View view) {
        ...
        Retrofit retrofit = new Retrofit.Builder().baseUrl("https://api.thecatapi.com/")
                ...
                .build();
        ICatService catRequest = retrofit.create(ICatService.class);
        Call<List<Cat>> call = catRequest.getCats();
        call.enqueue(new Callback<List<Cat>>() {
            @Override
            public void onResponse(Call<List<Cat>> call, Response<List<Cat>> response) {
                if (response.body()!=null){
                    loadPics(response.body());
                    Log.v("request cats","request cats success");
                }
                restoreBtn();
            }
            ...
            public void onFailure(Call<List<Cat>> call, Throwable t) {
                restoreBtn();
                Log.v("request cats","request cats fail");
            }
        });
